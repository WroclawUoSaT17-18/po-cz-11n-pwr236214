...
// ile czasu na pytania
    final static long INTERVAL = 1000; // 1 second
    final static long TIMEOUT = 7000; // 7 sconds
    int progressValue = 0;

    CountDownTimer mCountDown; // żeby pokazywało ile zostało czasu
    ...
    public void onClick(View v) {
        ...
        if(index < totalQuestion){
            ...
            {// dodawnie pkt za np przykładową odp
                score+=10;
                correctAnswer++ ;
                showQuestion(++index);
            }
            else
                showQuestion(++index); // przeskakiwanie
